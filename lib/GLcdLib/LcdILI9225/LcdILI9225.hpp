...
private:
	...
	void	SendCommandWriteGRam() override { glcdSpi.SendCommand((uint8_t)ERegIdx::IdxWRGRAM); }
	void	SendRegVal(ERegIdx reg, uint16_t val) { glcdSpi.SendCommand((uint8_t)reg); glcdSpi.SendData(val); }
