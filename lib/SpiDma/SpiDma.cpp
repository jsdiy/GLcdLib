...
//インスタンス
//・ArduinoFrameworkがクラス'SPIClass'のインスタンス'SPI'を静的に生成しているのと同じ。
//・ただしSpiDmaはインスタンス名をアッパーキャメルケースにするとクラス名と同じになってしまうので（エラー）、ロワーキャメルケースとした。
SpiDma	spiDma;
...
void	SpiDma::InitializeSpiBusConfig(spi_bus_config_t* spiBusConfig, gpio_num_t mosi, gpio_num_t miso, gpio_num_t sck)
{
	...
	spiBusConfig->max_transfer_sz = MaxTxBufferSize;
