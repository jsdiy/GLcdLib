...
private:
	void	SetGRamArea(int16_t x, int16_t y, int16_t w, int16_t h)	override;
	void	SendCommandWriteGRam() override { SendCommand(Command::RAMWR); }
	void	SendCommand(Command cmd) { glcdSpi.SendCommand((uint8_t)cmd); }
	void	SendCommandValue(Command cmd, uint8_t val) { SendCommand(cmd); glcdSpi.SendData(val); }
