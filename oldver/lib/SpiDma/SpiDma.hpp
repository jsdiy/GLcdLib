...
{
public:
	static	constexpr	size_t	MaxTxBufferSize = 20480UL;
	static	constexpr	size_t	ToBitLength(size_t byteLength) { return byteLength * 8; }
	/*	spi_bus_config_t.max_transfer_sz に指定する値について
	SPI-DMAのデスクリプタ1つで4092byteまで転送可能
	max_transfer_sz=0とした場合は4092byteを指定したことになる
	max_transfer_szは、ESP32では64KiBまで指定可能, ESP32-C3では262143bit(=32KiB-1bit)まで指定可能
	*/
...
public:
	SpiDma() { spiMutex = xSemaphoreCreateMutex(); }
